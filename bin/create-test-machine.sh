...
DOMAIN=$(hostname)
mkdir -p tmp/web
"${HOME}/src/debian-tools/.venv/bin/dt" --hostname example --domain shiin.org --root-password root --user-name example --user-password example --user-real-name "Example User" --output-document tmp/web/example.cfg
cd tmp/web
