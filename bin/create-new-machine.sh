...
if [ "${SUDO_USER}" = "" ]; then
    ...
    if [ ! -d "${CONFIGURATION_DIRECTORY}/TFTP/debian-installer" ]; then
        tar --extract --file tmp/netboot.tar.gz --directory "${CONFIGURATION_DIRECTORY}/TFTP"
    fi
else
    ...
    if [ ! -d "${CONFIGURATION_DIRECTORY}/TFTP/debian-installer" ]; then
        sudo -u "${SUDO_USER}" tar --extract --file tmp/netboot.tar.gz --directory "${CONFIGURATION_DIRECTORY}/TFTP"
    fi
fi
...
sleep 1
bin/input.sh "${MACHINE_NAME}" "auto url=http://${ADDRESS}:8000/${MACHINE_NAME}.cfg"
sleep 1
...
elif [ "${BRIDGE_DEVICE}" = bridged ]; then
